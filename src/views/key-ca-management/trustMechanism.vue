<template>
  <Card :bordered="false" dis-hover>
    <Button style="marginLeft:30px;" @click="isShowInput=true" type="primary" icon="md-arrow-up">导入</Button>
    <div style="marginTop:20px;">
      ...
      >
        <template slot-scope="{ row,index }" slot="action">
          <Button class="ops-btn" type="primary" @click="showMechanism">查看</Button>
          <Button class="ops-btn" type="primary" @click="updateMechanism">配置</Button>
          <Poptip confirm title="确认删除该受信机构吗?" @on-ok="deleteMechanism(row.id)" placement="right">
            <Button class="ops-btn" type="error">删除</Button>
          </Poptip>
          <Button class="ops-btn" type="info" @click="downloadMechanism">下载</Button>
        </template>
      ...
      <Modal v-model="isShowMechanism" title="证书详细" :footer-hide=true>
        <Form ref="mechanismForm" :model="mechanismForm" :label-width="100">
          <FormItem label="证书类型：" prop="certType" style="height:15px">{{mechanismForm.certType}}</FormItem>
          <FormItem
            label="序列号："
            prop="serialNumber"
            style="height:15px"
          >{{mechanismForm.serialNumber}}</FormItem>
          <FormItem label="版本号：" prop="version" style="height:15px">{{mechanismForm.version}}</FormItem>
          <FormItem label="主题：" prop="theme" style="height:15px">{{mechanismForm.theme}}</FormItem>
          <FormItem label="颁发者：" prop="publisher" style="height:15px">{{mechanismForm.publisher}}</FormItem>
          <FormItem label="开始时间：" prop="startDate" style="height:15px">{{mechanismForm.startDate}}</FormItem>
          <FormItem label="失效时间：" prop="endDate" style="height:15px">{{mechanismForm.endDate}}</FormItem>
          <FormItem label="签名算法：" prop="alg" style="height:15px">{{mechanismForm.alg}}</FormItem>
          <FormItem label="密钥长度：" prop="keyLength" style="height:15px">{{mechanismForm.keyLength}}</FormItem>
          <FormItem label="密钥用途：" prop="keyType" style="height:15px">{{mechanismForm.keyType}}</FormItem>
          <FormItem label="基本约束：" prop="constraint" style="height:15px">{{mechanismForm.constraint}}</FormItem>
        </Form>
      </Modal>
      <Modal v-model="isUpdateMechanism" title="配置证书" :footer-hide=true>
        <Form ref="updateMechanismForm" :model="updateMechanismForm" :label-width="140">
          ...
          <FormItem label="CRL：" prop="crl">
            <Input v-model="updateMechanismForm.crl" type="textarea" style="width: 300px" />
          </FormItem>
