...
const rsaKey = {
    ...
    mutations: {
        queryRSAKeyList(state){
            state.loading = true;
            let reqData = {};
            reqData.data =4;
            axios.post(conf.apiServer + "key/getKeyList", reqData).then(
                res => {
    ...
    actions:{
        RSAKeyGeneration({ commit }, createKey) {
        ...
        deleteRSAKey({ commit }, deleteKey) {
            const data=new FormData();
            data.append("keyType",4);
